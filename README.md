...
The sealed v1 local-operator receipt `RR-WP0002-LOCAL-OPERATOR-20260717` and its amendment remain append-only historical evidence with disposition `superseded-unclosed-never-effective`; its absent `docs/evidence/WP-0002/local-operator-amendment/authority.json`, `docs/evidence/WP-0002/local-operator-amendment/pre-merge.json`, and `docs/evidence/WP-0002/local-operator-amendment/complete.json` paths are permanently forbidden. The v2 [`delegated local Unity operator successor`](docs/foundation-v0.1/governance/WP-0002-DELEGATED-LOCAL-UNITY-OPERATOR-SUCCESSOR.md) is the only forward route: amendment `A1B-WP-0002-LOCAL-OPERATOR-SUCCESSOR-20260718`, transaction `WP0002-LOCAL-OPERATOR-SUCCESSOR-20260718`, and receipt `RR-WP0002-LOCAL-OPERATOR-SUCCESSOR-20260718`. That successor remains non-executable until its authenticated online transaction and a separate protected evidence-closure PR containing exactly `docs/evidence/WP-0002/local-operator-successor/authority.json`, `docs/evidence/WP-0002/local-operator-successor/pre-merge.json`, and `docs/evidence/WP-0002/local-operator-successor/complete.json` both merge. Only for the exact successor control PR head and patch may `wp0002-policy` be temporarily nonrequired; `validate`, `wp0002-core`, and every non-status protection remain, exact-three protection must be restored within 600 seconds of the successor control squash, and the closure PR must then pass `validate`, `wp0002-core`, and `wp0002-policy`. All successor reports absent is pending; any partial set fails closed. The base-owned closure run live-reverifies both owner comments and the complete transaction. GitHub's classic protection endpoint requires a separately provisioned single-repository, short-expiry Administration-read Actions secret; its absence fails closed, and it is exposed only to protected-base closure code, never candidate code or an agent. Only after closure merges does the successor permit visible Computer Use for exact-project Hub/Editor open/switch, approval of the receipt-bound Bridge client whose visible OS publisher/path and application version/build match, and non-secret inspection. The prior signature observation remains context-limited; the UI does not prove a CDHash. Implementation still uses only the exact five MCP tools; Bridge tool/configuration mutation, direct Unity process invocation, and every prior denial remain in force.
...
Expected current result after the successor control transaction and evidence closure complete: the foundation lint passes with 64 decision records and 22 receipts, including five intentionally unsealed historical bootstrap captures and both append-only local-operator receipts.
