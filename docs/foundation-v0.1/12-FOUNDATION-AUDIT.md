...
3. The route is not activation-clean. Approval was manually granted to Codex 0.144.2, current Codex 0.144.5 inherited that approval, 31 handshakes advertised all 54 tools, and the relay's MCP-client port listens beyond loopback. The replacement requires a revoked/disconnected zero-call preflight and a distinct live creator-approved session, exact sanitized allowlist plus effective-server-inventory hashes, isolated Codex/XDG/Git environment, current process-birth identities, an actual Unix socket with process-FD ownership/peer proof, exact source-hashed creator-authorized raw collectors, fixed raw protocol captures, and a passing read-only live route capture. Any reconnect, restart, collector/target/config/environment/allowlist drift, or receipt delay invalidates it.
