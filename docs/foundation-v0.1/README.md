...
Current A0 activation preparation uses the creator-approved durable checkout `/Users/sasha/Documents/Codex/sasha-the-land-pirate` and remote `https://github.com/AC-21/sasha-the-land-pirate`. Sealed creator receipts accept the packet contract, temporary identity, repository, authorization to install the exact D-0047 candidate, and `UNITY-AI-GATEWAY-CODEX` route; D-0050 narrows installation to creator-operated setup and does not ratify the engine. Local inspection still has not proved the assigned Unity AI seat, same-organization project linkage, Gateway/bundled-client profile, exact installed Editor/IL2CPP/Xcode tuple, or standalone quarantine. WP-0001 is accepted but inactive; autonomy remains A0.
