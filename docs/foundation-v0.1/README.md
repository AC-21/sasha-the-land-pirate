...
Current A0 activation preparation uses the creator-approved durable checkout `/Users/sasha/Documents/Codex/sasha-the-land-pirate` and remote `https://github.com/AC-21/sasha-the-land-pirate`. The original sealed receipts still accept the exact packet contract, temporary identity, repository, and creator-operated installation of the D-0047 candidate. D-0051 supersedes only D-0049's route choice and selects `UNITY-MCP-EXTERNAL`: Codex as external client through Unity's MCP Bridge and exact Unity-installed relay, not the API-key Gateway path. Its current local decision receipt is unsealed and cannot satisfy the immutable packet's separate requirement for a creator-controlled external `packet-acceptance` route-amendment receipt. Local inspection also has not proved the assigned Unity AI seat, same-organization project linkage, exact relay/client/target/connection profile, exact installed Editor/IL2CPP/Xcode tuple, or standalone quarantine. WP-0001 is accepted but inactive; autonomy remains A0.
