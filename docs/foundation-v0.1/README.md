...
Current A0 activation preparation uses the creator-approved durable checkout `/Users/sasha/Documents/Codex/sasha-the-land-pirate` and remote `https://github.com/AC-21/sasha-the-land-pirate`. D-0049 and WP-0001 are protected by sealed creator receipts: the packet contract, temporary identity, repository, exact D-0047 installation, and `UNITY-AI-GATEWAY-CODEX` route are accepted. Local inspection still has not proved the assigned Unity AI seat, same-organization project linkage, Gateway/bundled-client profile, exact installed Editor/IL2CPP/Xcode tuple, or standalone quarantine. WP-0001 is accepted but inactive; autonomy remains A0.
