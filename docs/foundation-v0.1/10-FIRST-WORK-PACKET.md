...
The first implementation is split across two separately authorized packets:
...
- primitive placement/production;
- exact human-only, robot-only, and mixed resident/staffing state without a hidden mandatory human;
- primitive feel test for the creator-selected road-control embodiment;
- persistent city/road ownership transaction;
- stable IDs, counter/key randomness, and Save Envelope v1;
- immutable-generation save/recovery, protected pruning-anchor retention, fault injection, and canonical-state comparison;
- batch test/build/report automation;
- slice-scale benchmark harness and representative dynamic-city rendering test;
...
The corrected WP-0001 proposal is frozen at contract `eed333603affe6aa1dd2b16b26ae702d9f561cc653fa319da02abfe008faeda3`. It adds .NET SDK 10.0.301 for locked headless tests while keeping Unity-consumed code at `netstandard2.1`/C# 9, makes `SimulationCore` and `SaveContracts` dual-use local Unity packages with no copied source, requires Editor-generated package locks, pins official `com.unity.ai.assistant` 2.14.0-pre.1 plus an eligible assigned Unity AI seat and linked project, denies direct agent Unity executable/CLI/batchmode use, requires native IL2CPP ARM64 and Metal evidence, and replaces the unsafe shared-worktree boundary with a standalone disposable clone/sandbox. Its acceptance receipt must choose exactly one evidence profile: Unity AI Gateway with Unity-bundled Codex and automatic connection approval, or external Unity MCP with the running Bridge, exact Unity-installed relay/client, and a creator-approved zero-tool-call setup handshake.

WP-0001 nevertheless remains proposed and blocked on protected acceptance, a verified/receipt-bound official Unity AI Gateway or MCP route under D-0048, the exact D-0047 Editor/IL2CPP/Xcode tuple, and a physically verified A1 quarantine/manual-import boundary. WP-0002 additionally waits on value-matching identity/core receipts, explicit city-comparison authorization, and its own acceptance in `governance/ratification-state.json`. Separate status histories prevent A0 setup from silently authorizing implementation.
